project folder: JonaXin-cs1c-project03\

Brief description of submitted files:

src/BrowserNavigation.java
    - Includes main() for debugging, read input from the text file in the resources folder. Template code provided by Bita M.

src/Navigator.java
    - Simulates how a browser works.

src/StackList.java
    - Includes methods that needed to build a stack, and two inner classes Node and StackIterator.

src/StackListTestMain.java
    - Test main() for StackList class.

resources/links.txt
    - The first test input file.

resources/popEmptyBacklinkStack.txt
    - The second test input file.

resources/popEmptyForwardlinkStack.txt
    - The third test input file.

README.txt
    - description of submitted files.
