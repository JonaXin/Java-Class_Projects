        ...
        implements Cloneable
{
    ...
    /**
     * The method that physically removes a node that is marked as deleted
     * from the tree by calling its private helper method.
     * @return true or false depending on whether or not a node is removed
     *         from the tree.
     */
     ...
     * @param root the tree root that starts finding the node.
    ...
    /**
     * The private recursive method for finding the node that contains
     * the smallest value in the tree, including the nodes that are
     * marked as deleted.
     * @param root the tree root that starts finding the node.
     * @return the node after found.
     */
    ...
    /**
     * The private recursive method for finding the node that contains
     * the biggest value in the tree, including the nodes that are
     * marked as deleted.
     * @param root the tree root that starts finding the node.
     * @return the node after found.
     */
    ...
    /**
     * The method that physically removes a node from the tree by
     * replacing the data and other references from another node to
     * the current the root, or simply set the root to be null,
     * depending on whether or not the root is a leaf or has a left
     * child or right child only. We don't need worry about the case
     * when a root has a right child and a left child since this
     * situation is covered in previous condition.
     * @param root the root that starts for searching a deleted node.
     * @param x the data that needs to be removed from the tree.
    ...
    protected LazySTNode removeHard( LazySTNode root, E x  )
    {
        ...
        {
            root.deleted = false;
            root.data = findMaxHard(root.lftChild).data;
            root.lftChild = removeHard(root.lftChild, root.data);
        }
    ...
    /**
     * The private recursive method that helps physically remove a
     * node from the tree. Starts by finding the node that is marked
     * as deleted in the lowest height and call the removeHard()
     * method to delete it.
     * @param root the root that starts "collecting garbage".
     * @return a root node.
     */
